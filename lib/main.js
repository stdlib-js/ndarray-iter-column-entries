...
function nditerColumnEntries( x ) {
	...
	if ( !isndarrayLike( x ) ) {
		throw new TypeError( format( '1lL4f', x ) );
	}
	opts = {
		'writable': false
	};
	if ( arguments.length > 1 ) {
		...
		if ( !isPlainObject( options ) ) {
			throw new TypeError( format( '1lL2V', options ) );
		}
		if ( hasOwnProp( options, 'readonly' ) ) {
			if ( !isBoolean( options.readonly ) ) {
				throw new TypeError( format( '1lL2o', 'readonly', options.readonly ) );
			}
			opts.writable = !options.readonly;
			if ( opts.writable && isReadOnly( x ) ) {
				throw new Error( format( '1lLF4' ) );
			}
		}
	}
	...
	if ( ndims < 2 ) {
		throw new TypeError( format('1lLF2') );
	}
